...
function StepIndicator({ step, current }: { step: number; current: number }) {
  ...
  return (
    <div className={`
      ...
      ${done ? "bg-emerald-600 border-emerald-600 text-white dark:bg-emerald-500 dark:border-emerald-500" :
        active ? "bg-slate-900 border-slate-900 text-white dark:bg-slate-50 dark:border-slate-50 dark:text-slate-900" :
  ...
  return (
    <div className="space-y-3">
      ...
      >
        ...
        <Badge variant="secondary" className="text-[10px] font-mono uppercase tracking-wider dark:bg-slate-800 dark:text-slate-300">
          Click or drag &amp; drop
        </Badge>
      </div>
...
export default function StockTransferCreate() {
  ...
  const addItem = (product: PosProduct, specificVariantId?: string) => {
    ...
    const safeProductName =
      product.productName || (product as any).product_name || product.name || "Unknown Product"
    ...
    if (!selectedVariant) { toast.error("Could not determine product variant"); return }
  ...
  const handleSystemFileReceived = (filePath: string) => {
    ...
    if (attachedFiles.some((f) => f.path === filePath)) { toast.info("File already attached"); return }
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-2 select-none">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-sm font-medium text-slate-700 dark:text-slate-300">{currentLocation?.name || "Origin"}</span>
            </div>
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className={`text-sm font-medium ${selectedToBranch ? "text-slate-700 dark:text-slate-300" : "text-slate-400 dark:text-slate-500"}`}>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-start justify-between">
            <div>
              <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-50 tracking-tight">Create Stock Transfer</h1>
            ...
            {[
              ...
              <div key={s.n} className="flex items-center">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className={`text-xs font-medium hidden sm:block ${currentStep >= s.n ? "text-slate-800 dark:text-slate-200" : "text-slate-400 dark:text-slate-500"}`}>
                    {s.label}
                  </span>
                </div>
                {i < arr.length - 1 && (
                  <div className={`h-px w-12 mx-3 ${currentStep > s.n ? "bg-emerald-500" : "bg-slate-200 dark:bg-slate-800"}`} />
                )}
              </div>
            ))}
          </div>
          ...
          <div className="space-y-5">
            ...
            <div className="bg-white dark:bg-background rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-colors">
              <div className="flex items-center gap-3 px-6 py-4 border-b border-slate-100 dark:border-slate-800 select-none">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 dark:text-slate-500">Select the branch that will receive this stock</p>
                </div>
              </div>
              <div className="p-6">
                <div className="grid sm:grid-cols-2 gap-4 items-end">
                  <div className="space-y-1.5">
                    ...
                    <div className="flex items-center gap-2.5 h-10 px-3 rounded-lg bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700">
                      ...
                      <Badge variant="secondary" className="ml-auto text-[10px] dark:bg-slate-800 dark:text-slate-300 select-none">Origin</Badge>
                    </div>
                  </div>
                  <div className="space-y-1.5">
                    <Label className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider select-none">
                    ...
                    <Select value={toBranch} onValueChange={setToBranch} disabled={isSubmitting}>
                      <SelectTrigger className="h-10 text-sm border-slate-200 dark:border-slate-700 focus:ring-slate-900 dark:focus:ring-slate-50 dark:bg-background">
                        <SelectValue placeholder={isLoadingLocations ? "Loading locations…" : "Select destination branch"} />
                      </SelectTrigger>
                      <SelectContent>
            ...
            <div className="bg-white dark:bg-background rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-colors">
              <div className="flex items-center gap-3 px-6 py-4 border-b border-slate-100 dark:border-slate-800 select-none">
                ...
                {items.length > 0 && (
                  <Badge variant="outline" className="text-xs border-slate-300 dark:border-slate-700 dark:text-slate-300">
                    ...
                    <div className="absolute top-full left-0 right-0 z-50 mt-1.5 rounded-lg border border-slate-200 dark:border-slate-800 shadow-xl dark:shadow-2xl bg-white dark:bg-background overflow-hidden">
                      {isLoadingProducts ? (
                        ...
                        <div className="py-10 text-center select-none">
                          ...
                          <p className="text-sm text-slate-500 dark:text-slate-400">No products found for "{searchTerm}"</p>
                        </div>
                      ) : (
                        <ScrollArea className="max-h-80">
                          <div className="divide-y divide-slate-50 dark:divide-slate-800/50">
                            {searchResults.map((product) => {
                              ...
                              const safeProductName = product.productName || (product as any).product_name || product.name || "Unknown"

                              if (available.length === 0) {
                                return (
                                  <div key={product.productId} className="flex items-center justify-between px-4 py-3 opacity-50 select-none">
                                ...
                                return (
                                  <div key={product.productId}>
                                    <div className="flex items-center gap-2 px-4 py-2 bg-slate-50 dark:bg-slate-800/50 select-none">
                                      ...
                                      <span className="text-xs font-semibold text-slate-600 dark:text-slate-400">{safeProductName}</span>
                                    </div>
                                    {available.map((variant) => (
                                      ...
                                      >
                                        <div>
                                          <p className="text-sm font-medium text-slate-800 dark:text-slate-200">{variant.variantName}</p>
                                          <p className="text-xs text-slate-400 dark:text-slate-500">Stock: {variant.stock}</p>
                                        </div>
                                        <Button size="sm" variant="outline" className="h-7 text-xs dark:border-slate-700 dark:bg-background dark:hover:bg-slate-800">Add</Button>
                                      </div>
                                    ))}
                                  </div>
                                )
                                ...
                                >
                                  <div>
                                    ...
                                    <div className="flex gap-3 text-xs text-slate-400 dark:text-slate-500 mt-0.5">
                                      <span>Stock: <b className="text-slate-600 dark:text-slate-400">{v.stock}</b></span>
                                      {v.sku && <span>SKU: {v.sku}</span>}
                                    </div>
                                  </div>
                                  <Button size="sm" className="h-7 text-xs bg-slate-900 hover:bg-slate-700 dark:bg-slate-50 dark:hover:bg-slate-200 text-white dark:text-slate-900">Add</Button>
                                </div>
                              )
                            })}
                          </div>
                ...
                {items.length === 0 ? (
                  <div className="flex flex-col items-center justify-center gap-3 py-14 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-lg bg-slate-50/50 dark:bg-slate-800/20 select-none">
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">Use the search above to add products</p>
                    </div>
                  </div>
                ) : (
                  <div>
                    ...
                    <div className="grid grid-cols-[1fr_auto_auto] gap-4 items-center px-3 pb-2 select-none">
                      <span className="text-[10px] uppercase tracking-wider font-semibold text-slate-400 dark:text-slate-500">Product</span>
                      <span className="text-[10px] uppercase tracking-wider font-semibold text-slate-400 dark:text-slate-500 text-center w-28">Qty</span>
                      <span className="w-8" />
                    </div>
                          ...
                          >
                            <div>
                              ...
                              <div className="flex items-center gap-2 mt-0.5 select-none">
                                ...
                                <span className={`text-[10px] font-medium ${item.currentStock <= 5 ? "text-amber-500 dark:text-amber-400" : "text-slate-400 dark:text-slate-500"}`}>
              ...
              <div className="p-5 space-y-5">
                ...
                <div className="grid grid-cols-2 gap-3 select-none">
                  <div className="rounded-lg bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700/50 p-3 text-center">
                    ...
                    <div className="text-[10px] uppercase tracking-wider text-slate-400 dark:text-slate-500 mt-0.5 font-medium">SKUs</div>
                  </div>
                  <div className="rounded-lg bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700/50 p-3 text-center">
                    <div className="text-2xl font-bold text-slate-900 dark:text-slate-100">{getTotalItems()}</div>
                    <div className="text-[10px] uppercase tracking-wider text-slate-400 dark:text-slate-500 mt-0.5 font-medium">Total Units</div>
                  </div>
                </div>
                ...
                {(currentLocation || selectedToBranch) && (
                  <div className="rounded-lg border border-slate-100 dark:border-slate-800 p-3 space-y-2 select-none">
                    <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">Transfer Route</p>
                    <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      <div className={`h-1.5 w-1.5 rounded-lg flex-shrink-0 ${selectedToBranch ? "bg-sky-500" : "bg-slate-200 dark:bg-slate-700"}`} />
                      <span className={`text-xs font-medium truncate ${selectedToBranch ? "text-slate-700 dark:text-slate-300" : "text-slate-400 dark:text-slate-500"}`}>
                ...
                {/* Notes */}
                <div className="space-y-1.5">
                  <Label className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider select-none">Notes</Label>
                  <Textarea

...
const settingsItems = [
  { id: 'general', name: 'General Settings', description: 'Store information and preferences', icon: Settings, path: '/settings?tab=general' },
  { id: 'sync', name: 'Sync Settings', description: 'Product and customer synchronization', icon: RefreshCw, path: '/settings?tab=sync' },
  { id: 'data', name: 'Data Management', description: 'Clear local data and reset', icon: Database, path: '/settings?tab=data' },
  { id: 'receipts', name: 'Receipt Settings', description: 'Customize receipt layout', icon: FileText, path: '/settings?tab=receipts' },
  { id: 'hardware', name: 'Hardware Settings', description: 'Printers and peripherals', icon: Printer, path: '/settings?tab=hardware' },
  { id: 'notifications', name: 'Notification Settings', description: 'Configure alerts', icon: Bell, path: '/settings?tab=notifications' },
  { id: 'display', name: 'Customer Display', description: 'External display settings', icon: Monitor, path: '/settings?tab=display' },
]
...
export function GlobalSearch() {
  ...
  const hasResults = results.products.length > 0 || results.customers.length > 0 || results.sales.length > 0 || filteredSettings.length > 0
                    ...
                    {results.products.map((product) => (
                        ...
                        >
                            <div className="flex items-center justify-center w-10 h-10 bg-muted rounded-md shrink-0">
                              {product.imageUrl ? (
                                <img src={product.imageUrl} alt={product.productName} className="w-full h-full object-cover rounded-md" />
                              ) : (
                        ...
                        >
                            <div className={cn(
                              ...
                              <Receipt className={cn(
                                "w-5 h-5",
                                sale.status === 'FAILED' ? 'text-red-500' : 'text-yellow-600'
                              )} />
                            </div>
                            <div className="flex-1 min-w-0">

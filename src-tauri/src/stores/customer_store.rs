...
pub async fn run_sync(
    ...
    let target_url = format!(
        "{}/{}",
        clean_base_url,
        crate::api_config::routes::CUSTOMERS
    );
...
pub fn search_local(state: &CustomerState, query: String) -> Vec<PosCustomer> {
    ...
    customers
        ...
        .filter(|c| {
            ...
            let matches_company = c
                .company
                .as_ref()
                .is_some_and(|comp| comp.to_lowercase().contains(&query));
...
pub async fn create_customer(
    ...
    let raw_val: serde_json::Value = response
        .json()
        .await
    ...
    let new_customer: PosCustomer = if raw_val.get("data").is_some() {
        ...
        serde_json::from_value(raw_val).context("Failed to parse response into PosCustomer")?
    };
